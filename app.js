...
function removeNetlifyBadge(){
  const selectors=['[data-netlify-badge]','[data-netlify-ui]','.netlify-badge','.netlify-drawer','iframe[src*="netlify.com"][title*="Netlify" i]','iframe[src*="netlify.app"][title*="Netlify" i]'];
  selectors.forEach(s=>document.querySelectorAll(s).forEach(el=>el.remove()));
  document.querySelectorAll('a,button,div,span').forEach(el=>{
    const text=(el.textContent||'').trim().toLowerCase();
    if(text==='powered by netlify'||text.startsWith('powered by netlify'))(el.closest('a,button,[role="button"],div')||el).remove();
  });
}
...
function warmImage(src,priority='low'){
  ...
  warmed.add(src);
  const img=new Image();img.decoding='async';img.fetchPriority=priority;img.src=src;
}
...
function warmNeighbors(imgs,index){
  if(!canPrefetch()||imgs.length<2)return;
  warmImage(imgs[(index+1)%imgs.length],'low');
}
function warmProduct(p){warmImage(firstImage(p),'low')}
...
function renderProducts(){
  ...
  $('#products').innerHTML=list.map((p,n)=>{
    ...
    return `<article class="product-card reveal-card" role="button" tabindex="0" data-id="${p.id}"><div class="product-image-wrap"><img src="${src}" alt="${safeText(p.name)}" loading="${n===0?'eager':'lazy'}" decoding="async" fetchpriority="${n===0?'high':'low'}"><span class="badge">NEW DROP</span><span class="photo-count">${names.length} colors</span></div><div class="product-info"><div><h3>${safeText(p.name)}</h3><p>${names.join(' · ')} · Sizes 37–41</p></div><div class="product-price">${money(p.price)}</div></div></article>`;
...
function openProduct(p){
  if(!p)return;
  const variants=usableVariants(p),colors=Object.keys(variants);if(!colors.length)return;
  let color=colors[0],imgs=variants[color],index=0,selectedSize='';
  ...
  function renderGallery(direction=0){
    ...
    if(mainImg.src!==new URL(imgs[index],location.href).href)mainImg.src=imgs[index];
    mainImg.classList.add(direction>0?'slide-left':direction<0?'slide-right':'image-pop');
    counter.textContent=`${index+1} / ${imgs.length}`;
    thumbs.innerHTML=imgs.map((src,n)=>`<button class="thumb ${n===index?'active':''}" data-index="${n}" aria-label="View photo ${n+1}">${n===index?`<img src="${src}" alt="" decoding="async">`:`<span class="thumb-number">${n+1}</span>`}</button>`).join('');
    $$('.thumb').forEach(t=>t.addEventListener('click',()=>{const old=index;index=Number(t.dataset.index);renderGallery(index>old?1:-1)}));
    warmNeighbors(imgs,index);
  }
  function move(delta){index=(index+delta+imgs.length)%imgs.length;renderGallery(delta>0?1:-1)}
  renderGallery();
  $('#galleryPrev').addEventListener('click',e=>{e.stopPropagation();move(-1)});
  $('#galleryNext').addEventListener('click',e=>{e.stopPropagation();move(1)});
  ...
  area.addEventListener('touchend',e=>{if(!e.changedTouches.length)return;const dx=e.changedTouches[0].clientX-tx,dy=e.changedTouches[0].clientY-ty;if(Math.abs(dx)>=34&&Math.abs(dx)>Math.abs(dy)*1.15)move(dx<0?1:-1)},{passive:true});
  ...
  $$('.color-btn').forEach(btn=>btn.addEventListener('click',()=>{
    color=btn.dataset.color;imgs=variants[color];index=0;
    $$('.color-btn').forEach(x=>x.classList.toggle('active',x===btn));
    $('#colorHint').textContent=color;renderGallery();
  }));
  ...
  $('#addToBag').addEventListener('click',()=>{
    if(!selectedSize){$('#sizeHint').textContent='Choose a size first';$('#sizeHint').classList.add('error-text');return}
    const existing=state.cart.find(x=>x.id===p.id&&x.size===selectedSize&&x.color===color);
    if(existing)existing.qty=Math.min(9,(existing.qty||1)+1);else state.cart.push({id:p.id,size:selectedSize,color,qty:1});
    saveCart();const btn=$('#addToBag');btn.innerHTML='Added to bag <span>✓</span>';btn.disabled=true;
    setTimeout(()=>{if(btn){btn.innerHTML='Add to bag <span>+</span>';btn.disabled=false}},900);
  });
  ...
  modal.showModal();modal.classList.remove('closing');
}
...
function renderCart(){
  ...
  $('#cartItems').innerHTML=state.cart.length?state.cart.map((x,n)=>{
    const p=products.find(z=>z.id===x.id)||products[0],vars=usableVariants(p),src=(vars[x.color]||Object.values(vars)[0]||[])[0]||'';
    return `<div class="cart-row"><img src="${src}" alt="${safeText(p.name)}" loading="lazy" decoding="async"><div class="cart-copy"><h4>${safeText(p.name)}</h4><p>${safeText(x.color)} · Size ${safeText(x.size)}</p><strong>${money(p.price)}</strong><div class="qty-control"><button data-index="${n}" data-delta="-1">−</button><span>${x.qty||1}</span><button data-index="${n}" data-delta="1">+</button></div></div><button class="remove-btn" data-index="${n}">×</button></div>`;
  }).join(''):'<div class="empty bag-empty"><b>Your bag is empty.</b><span>Pick a color and choose your size.</span></div>';
  $$('.qty-control button').forEach(b=>b.addEventListener('click',()=>{const i=Number(b.dataset.index),d=Number(b.dataset.delta);state.cart[i].qty=Math.max(0,Math.min(9,(state.cart[i].qty||1)+d));if(!state.cart[i].qty)state.cart.splice(i,1);saveCart()}));
  $$('.remove-btn').forEach(b=>b.addEventListener('click',()=>{state.cart.splice(Number(b.dataset.index),1);saveCart()}));
...
$('#modalClose').addEventListener('click',()=>animateClose($('#productModal')));$('#productModal').addEventListener('click',e=>{if(e.target===$('#productModal'))animateClose($('#productModal'))});$('#productModal').addEventListener('cancel',e=>{e.preventDefault();animateClose($('#productModal'))});
$('#searchBtn').addEventListener('click',()=>{$('#searchModal').showModal();lock();setTimeout(()=>$('#searchInput').focus(),30)});$('#searchClose').addEventListener('click',()=>{$('#searchModal').close();lock()});
...
$$('.filter').forEach(b=>b.addEventListener('click',()=>{$$('.filter').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.filter=b.dataset.filter;renderProducts()}));
$('#footerWhatsApp').href=waLink('Hi Sharif Store, I have a question.');
...
setTimeout(removeNetlifyBadge,500);setTimeout(removeNetlifyBadge,1500);
